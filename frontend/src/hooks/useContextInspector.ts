...
export function useContextInspector(enabled: boolean, onActionProposed: (action: ActionRecord) => void) {
  ...
  const [reviewFilters, setReviewFilters] = useState<ContextReviewFilters>({ decisions: ['pending'] })
  const [selectedReviewId, setSelectedReviewId] = useState<string | null>(null)
  const [reviewDetail, setReviewDetail] = useState<ContextReview | null>(null)
  const [reviewRecords, setReviewRecords] = useState<ContextRecordDetail[]>([])
  ...
  const selectReview = useCallback(
    async (reviewId: string | null): Promise<void> => {
      ...
      setReviewRecords([])
      setReviewRefreshRequired(false)
      if (!reviewId || !enabled) return
      setIsReviewLoading(true)
      try {
        const review = asReview(
          await fetch(API_ENDPOINTS.cortexContextReview(reviewId)).then(bodyOrError),
        )
        if (!review) throw new Error('Context review is unavailable.')
        const details = await Promise.all(
          reviewRecordIds(review).map(async (id) =>
            asDetail(await fetch(API_ENDPOINTS.cortexContextRecord(id)).then(bodyOrError)),
          ),
        )
        if (request !== reviewRequest.current) return
        setReviewDetail(review)
        setReviewRecords(details.filter((item): item is ContextRecordDetail => item !== null))
        setError(null)
      } catch (cause) {
        if (request === reviewRequest.current) {
          setError(cause instanceof Error ? cause.message : 'Context review could not be reached.')
        }
      } finally {
        if (request === reviewRequest.current) setIsReviewLoading(false)
      }
    },
    [enabled],
  )
  ...
  const decideReview = useCallback(
    async (decision: 'accept' | 'reject' | 'refresh'): Promise<boolean> => {
      if (!reviewDetail || reviewMutation) return false
      setReviewMutation(decision)
      try {
        const next = asReview(
          await fetch(API_ENDPOINTS.cortexContextReviewDecision(reviewDetail.id, decision), {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ expected_revisions: reviewDetail.expected_revisions }),
          }).then(bodyOrError),
        )
        if (!next) throw new Error('Context review is unavailable.')
        setError(null)
        setReviewRefreshRequired(false)
        await refresh()
        ...
        if (cause instanceof ContextRequestError && cause.status === 409) {
          await selectReview(reviewDetail.id)
          setReviewRefreshRequired(true)
        }
        setError(cause instanceof Error ? cause.message : 'Context review could not be updated.')
        return false
      } finally {
        setReviewMutation(null)
      }
    },
    [refresh, reviewDetail, reviewMutation, selectReview],
  )
