import { act, renderHook, waitFor } from '@testing-library/react'
import { afterEach, describe, expect, it, vi } from 'vitest'

import { useCortex } from './useCortex'

const agent = { key: 'local', display_name: 'Apex Felis' }

describe('useCortex Agent lifecycle', () => {
  afterEach(() => vi.restoreAllMocks())

  it('refreshes Agent status and rechecks it after a local model action', async () => {
    let statusRequests = 0
    vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
      const url = String(input)
      if (url.endsWith('/api/v1/agents')) {
        statusRequests += 1
        return new Response(JSON.stringify([agent]))
      }
      if (url.endsWith('/api/v1/cortex/local-model/load')) {
        expect(init?.method).toBe('POST')
        return new Response(null, { status: 204 })
      }
      throw new Error(`Unexpected request: ${url}`)
    })
    const { result } = renderHook(() => useCortex(false))
    await act(async () => { await result.current.refreshAgentsStatus() })
    expect(result.current.agentsStatus).toEqual([agent])
    expect(result.current.agentsStatusHydrated).toBe(true)
    await act(async () => { await result.current.loadLocalModel() })
    await waitFor(() => expect(statusRequests).toBe(2))
  })
})
