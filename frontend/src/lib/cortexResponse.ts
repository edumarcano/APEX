...
export function parseAgentQueryResponse(body: unknown): AgentQueryResponseBody {
  ...
  const metadata: AgentQueryMetadata | undefined = metadataRecord ? {
    agent: agentRecord && (agentRecord.key === 'cloud' || agentRecord.key === 'local' || agentRecord.key === 'panthera' || agentRecord.key === 'felis') ? { key: (agentRecord.key === 'panthera' ? 'cloud' : agentRecord.key === 'felis' ? 'local' : agentRecord.key) as AgentKey, version: nullableString(agentRecord.version), provider: nullableString(agentRecord.provider), configuredModel: nullableString(agentRecord.configured_model ?? agentRecord.configuredModel), resolvedModel: nullableString(agentRecord.resolved_model ?? agentRecord.resolvedModel), requestedEffort: nullableString(agentRecord.requested_effort ?? agentRecord.requestedEffort) as CloudEffort | null, resolvedEffort: nullableString(agentRecord.resolved_effort ?? agentRecord.resolvedEffort) } : null,
