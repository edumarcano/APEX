        ...
        <div className="flex-1 space-y-4 overflow-y-auto p-6">
          {history.length === 0 && !isQuerying ? (
            <p className="text-sm text-zinc-500">
              Ask APEX a question to start a Cortex session.
            </p>
          ) : null}
          ...
          {history.map((message, index) => {
            ...
            if (message.role === 'model') {
              const showTrace = index === lastModelIndex && latestTrace.length > 0
              return (
                <div key={`model-${index}`} className="flex justify-start">
                  <div className="max-w-[92%] rounded-2xl rounded-bl-md border border-white/10 bg-zinc-900/80 px-4 py-3">
                    ...
                    {showTrace ? <ToolTracePanel trace={latestTrace} /> : null}
                  </div>
                </div>
              )
            }
          ...
          {isQuerying ? (
            <div className="flex items-center gap-3 px-1 py-2">
              <Loader2 className="size-4 animate-spin text-[#39FF88]" aria-hidden />
