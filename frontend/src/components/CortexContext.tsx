...
function CaptureForm({ label, onSubmit, disabled }: { label: string; onSubmit: (value: ContextCaptureInput) => Promise<boolean>; disabled: boolean }): ReactElement {
  ...
  return <div className="space-y-2 rounded-lg border border-white/10 bg-white/[0.02] p-3">
    ...
    <button type="button" disabled={disabled || saving || !text.trim()} onClick={() => void submit()} className="rounded border border-[#7EB3FF]/45 px-2.5 py-1.5 font-mono text-[10px] uppercase tracking-wide text-[#7EB3FF] hover:text-white disabled:opacity-45">{saving ? 'Proposing…' : 'Propose action'}</button>
  </div>
}
