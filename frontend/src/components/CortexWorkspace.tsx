...
import { type AgentMessage, type AssistantQueryMetadata, type ToolTraceItem } from '../hooks/useApexAssistant'
import type { AgentProfileStatus, AssistantProfile, CloudEffort, LocalCommandStatus, LocalContextUsage, LocalToolScope, ToolOutputItem } from '../types/telemetry'
...
interface CortexWorkspaceProps {
  ...
  onLoadLocalModel: (profile: Extract<AssistantProfile, 'mus' | 'sorex'>) => Promise<boolean>
...
function isLocalProfile(profile: AssistantProfile): profile is Extract<AssistantProfile, 'mus' | 'sorex'> {
  return profile === 'mus' || profile === 'sorex'
}
...
function ResponseMetrics({ metadata }: { metadata: AssistantQueryMetadata | undefined }): ReactElement | null {
  ...
  return <div className="mt-3 space-y-3">
    {profile ? <div className="flex flex-wrap gap-x-3 gap-y-1 border-t border-white/10 pt-3 font-mono text-[10px] text-zinc-500"><span>{profile.provider ?? 'provider'} / {profile.key}</span><span>{profile.resolvedModel ?? profile.configuredModel ?? 'model unavailable'}</span>{profile.resolvedEffort ? <span>{profile.resolvedEffort} effort</span> : null}{profile.version ? <span>v{profile.version}</span> : null}</div> : null}
...
function LocalModelLifecycle({ profile, busy, actionPending, onLoad, onUnload }: { profile: AgentProfileStatus; busy: boolean; actionPending: boolean; onLoad: (profile: Extract<AssistantProfile, 'mus' | 'sorex'>) => Promise<boolean>; onUnload: () => Promise<boolean> }): ReactElement {
  ...
  return <section className="space-y-2" aria-label="Local model lifecycle"><p className="font-orbitron text-[10px] uppercase tracking-[0.16em] text-zinc-500">Local model</p><div className="rounded-lg border border-orange-500/25 bg-orange-950/10 p-3"><div className="flex items-center justify-between gap-2"><span className="font-mono text-[10px] uppercase tracking-wider text-orange-200">{lifecycleState}</span><span className="font-mono text-[10px] text-zinc-500">{profile.configured_model}</span></div><button type="button" disabled={disabled} onClick={() => void action()} className="mt-3 inline-flex w-full items-center justify-center rounded-md border border-orange-400/40 bg-orange-950/25 px-3 py-2 font-mono text-[10px] uppercase tracking-wider text-orange-100 transition-colors hover:border-orange-300 hover:bg-orange-950/45 disabled:cursor-not-allowed disabled:opacity-45">{transition ? <Loader2 className="mr-1.5 size-3.5 animate-spin" aria-hidden /> : null}{transition ? (canUnload ? 'Unloading…' : 'Loading…') : actionLabel}</button><p className="mt-2 text-[11px] leading-relaxed text-zinc-500">Loading pre-warms this profile; normal requests can still load it when needed. Unloading frees memory without changing the selected profile.</p>{!profile.active && lifecycleState === 'Unloaded' ? <p className="mt-1 text-[10px] text-zinc-600">Loading this profile may replace another resident local model.</p> : null}{busy ? <p className="mt-1 text-[10px] text-amber-200">Lifecycle actions are unavailable while local inference is active.</p> : null}{profile.reason && lifecycleState === 'Unavailable' ? <p className="mt-1 text-[10px] text-red-200">{profile.reason}</p> : null}{error ? <p className="mt-1 text-[10px] text-red-200" role="alert">{error}</p> : null}</div></section>
}

export function CortexWorkspace(props: CortexWorkspaceProps): ReactElement {
  const local = isLocalProfile(props.activeProfile)
