...
export function AgentQueryBar({
  ...
  const cortexStateClassName = queryActive
    ? 'border-[#A855F7]/70'
    : cortex && error
      ? 'agent-query-bar--error'
      : 'border-white/15'
  const formClassName = cortex
    ? [
      'agent-query-bar relative hud-command-surface w-full min-h-12 rounded-xl border bg-zinc-950/65 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] backdrop-blur-md',
