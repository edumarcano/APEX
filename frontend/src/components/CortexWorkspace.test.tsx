...
describe('CortexWorkspace', () => {
  ...
  it('shows only the selected profile until its anchored selector is opened', async () => {
    ...
    render(<CortexWorkspace {...workspaceProps({ onProfileChange, profilesStatus: [panthera, mus] })} />)
    ...
    const profileDialog = screen.getByRole('dialog', { name: 'Select profile' })
    expect(profileDialog).toBeInTheDocument()
    expect(profileDialog).toHaveAttribute('id', 'cortex-profile-popover')
  ...
  it('moves local scopes and context diagnostics into the inspector and arms one next request', async () => {
    ...
    const user = userEvent.setup()
    render(<CortexWorkspace {...workspaceProps({ activeProfile: 'mus', profilesStatus: [mus], commands: [weather], armedToolScope: 'weather', onArmedToolScopeChange })} />)
  ...
  it('shows local lifecycle controls only for local profiles and disables them during activity', () => {
    ...
    const { rerender } = render(<CortexWorkspace {...workspaceProps({ activeProfile: 'mus', profilesStatus: [mus], onLoadLocalModel })} />)
    ...
    rerender(<CortexWorkspace {...workspaceProps({ activeProfile: 'mus', profilesStatus: [mus], lifecycleBusy: true, onLoadLocalModel })} />)
