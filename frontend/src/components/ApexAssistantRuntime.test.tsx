...
describe('ApexAssistantRuntime', () => {
  it('loads the authoritative thread and gates a prompt through the APEX turn endpoint', async () => {
    ...
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
      ...
      if (url.endsWith(`/api/v1/cortex/conversations/${conversationId}/turns`)) {
        ...
        expect(payload.agent).toBe('cloud')
    ...
    render(
      <ApexAssistantRuntime
        config={{ agent: 'cloud', effort: 'medium', selectedToolNames: [], toolProfileId: null, snapshotId: null }}
        beforeRun={beforeRun}
      >
        <ApexAssistantThread />
      </ApexAssistantRuntime>,
  ...
  it('hides the in-flight Agent card and uses one Apex prefix in the working label', async () => {
    ...
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
      const url = String(input)
      if (url.endsWith('/api/v1/cortex/conversations?archived=true')) return response([])
      if (url.endsWith('/api/v1/cortex/conversations')) return response([{ ...summary, agent: 'local' }])
      if (url.endsWith(`/api/v1/cortex/conversations/${conversationId}`)) return response({ ...summary, agent: 'local', active_leaf_message_id: null, messages: [] })
      if (url.endsWith(`/api/v1/cortex/conversations/${conversationId}/turns`)) {
        expect(init?.method).toBe('POST')
        return turnResponse
      }
    ...
    render(
      <ApexAssistantRuntime
        config={{ agent: 'local', effort: null, selectedToolNames: [], toolProfileId: null, snapshotId: null }}
  ...
  it('keeps a user edit composer mounted across host rerenders', async () => {
    ...
    const { rerender } = render(
      <ApexAssistantRuntime config={{ agent: 'cloud', effort: 'medium', selectedToolNames: [], toolProfileId: null, snapshotId: null }}>
        <ApexAssistantThread />
      </ApexAssistantRuntime>,
    )
    ...
    rerender(
      <ApexAssistantRuntime config={{ agent: 'cloud', effort: 'high', selectedToolNames: [], toolProfileId: null, snapshotId: null }}>
        <ApexAssistantThread />
      </ApexAssistantRuntime>,
    )
    expect(screen.getByPlaceholderText('Edit message…')).toHaveValue('Original prompt')
  ...
  it('does not persist an empty HUD thread and creates it only when the first turn is accepted', async () => {
    ...
    const beforeRun = vi.fn(async () => true)
    render(
      <ApexAssistantRuntime config={{ agent: 'cloud', effort: 'medium', selectedToolNames: [], toolProfileId: null, snapshotId: null }} beforeRun={beforeRun}>
        <ApexAssistantThread />
      </ApexAssistantRuntime>,
    )
    await waitFor(() => expect(screen.getByText('APEX is ready. Start a session with a focused question.')).toBeInTheDocument())
  ...
  it('returns the updated conversation preferences after a PATCH', async () => {
    ...
    const updatedSummary = { ...summary, agent: 'local' as const, selected_tool_names: ['reminders'], tool_profile_id: 'focused' }
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
      const url = String(input)
      if (url.endsWith('/api/v1/cortex/conversations?archived=true')) return response([])
      if (url.endsWith('/api/v1/cortex/conversations')) return response([summary])
    ...
    await waitFor(() => expect(screen.getByText('APEX is ready. Start a session with a focused question.')).toBeInTheDocument())
    const updated = await runtimeRef.current?.patchPreferences({ agent: 'local', selectedToolNames: ['reminders'], toolProfileId: 'focused' })
    expect(updated).toEqual({ conversationId, agent: 'local', selected_tool_names: ['reminders'], tool_profile_id: 'focused' })
    expect(fetchMock).toHaveBeenCalled()
  })
  ...
  it('locks message mutation controls while an APEX turn is running', async () => {
    ...
    const user = userEvent.setup()
    render(<ApexAssistantRuntime config={{ agent: 'cloud', effort: 'medium', selectedToolNames: [], toolProfileId: null, snapshotId: null }}><ApexAssistantThread /></ApexAssistantRuntime>)
  ...
  it('treats a persisted pending turn as globally running after reload', async () => {
    ...
    render(<ApexAssistantRuntime config={{ agent: 'cloud', effort: 'medium', selectedToolNames: [], toolProfileId: null, snapshotId: null }} onRunningChange={onRunningChange}><ApexAssistantThread /></ApexAssistantRuntime>)
    await waitFor(() => expect(screen.getByText('Agent working')).toBeInTheDocument())
    expect(onRunningChange).toHaveBeenCalledWith(true, 'cloud')
  ...
  it('does not initialize a transient thread when preflight rejects', async () => {
    ...
    const user = userEvent.setup()
    render(
      <ApexAssistantRuntime config={{ agent: 'cloud', effort: 'medium', selectedToolNames: [], toolProfileId: null, snapshotId: null }} beforeRun={async () => false}>
        <ApexAssistantThread />
      </ApexAssistantRuntime>,
    )
    await waitFor(() => expect(screen.getByText('APEX is ready. Start a session with a focused question.')).toBeInTheDocument())
      ...
      if (url.endsWith(`/api/v1/cortex/conversations/${convId}`)) {
        return response({
          ...
          messages: [
            ...
            { id: agentMsgId, parent_message_id: userMsgId, role: 'agent', content: 'All checks normal.', status: 'completed', agent: 'cloud', created_at: '2026-08-18T12:00:01Z', updated_at: '2026-08-18T12:00:01Z' },
          ],
        })
      }
      throw new Error(`Unexpected request: ${url}`)
      ...
      if (url.endsWith(`/api/v1/cortex/conversations/${conv1}`)) {
        return response({
          ...
          messages: [
            ...
            { id: msg1Agent, parent_message_id: msg1User, role: 'agent', content: 'Reply 1 in Alpha', status: 'completed', agent: 'cloud', created_at: '2026-08-18T12:00:01Z', updated_at: '2026-08-18T12:00:01Z' },
          ],
        })
      }
    ...
    vi.spyOn(globalThis, 'fetch').mockImplementation(async (input) => {
      ...
      if (url.endsWith(`/api/v1/cortex/conversations/${convId}`)) {
        return response({
          ...
          messages: [
            { id: agentMsgId, parent_message_id: userMsgId, role: 'agent', content: 'Agent reply arrived.', status: 'completed', agent: 'cloud', created_at: '2026-08-18T12:00:00Z', updated_at: '2026-08-18T12:00:00Z' },
