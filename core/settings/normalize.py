...
def _normalize_agent_settings(
    ...
    if isinstance(cloud_raw, dict):
        _record_unsupported_agent_fields(
            cloud_raw,
            allowed={"designation", "model", "effort", "personal_context_enabled", "hosted_tools"},
            path="ask_apex.cloud",
            layer_name=layer_name,
            errors=errors,
        )
        cloud: dict[str, Any] = {}
        designation = cloud_raw.get("designation")
        if isinstance(designation, str):
            cleaned = designation.strip()
            if 1 <= len(cleaned) <= 32:
                cloud["designation"] = cleaned
            elif designation.strip() == "":
                cloud["designation"] = "Panthera"
            else:
                _record_error(errors, "ask_apex.cloud.designation is not valid")
        elif designation is not None:
            _record_error(errors, "ask_apex.cloud.designation must be a string")
        ...
        if isinstance(model, str) and get_model_profile(model.strip()) is not None:
            ...
            if profile is not None and profile.runtime == "cloud":
                reconciled_model = reconcile_cloud_model(
                    model.strip(),
                    dev_mode=is_dev_mode(),
                )
                cloud["model"] = reconciled_model
            else:
                _record_error(errors, "ask_apex.cloud.model is not valid")
        elif model is not None:
            _record_error(errors, "ask_apex.cloud.model is not valid")
        effort = cloud_raw.get("effort")
        if isinstance(effort, str):
            effort_str = effort.strip().lower()
            if effort_str in VALID_CLOUD_EFFORTS:
                cloud["effort"] = effort_str
            else:
                _record_error(errors, "ask_apex.cloud.effort is not valid")
        elif effort is not None:
            _record_error(errors, "ask_apex.cloud.effort is not valid")
        if "personal_context_enabled" in cloud_raw:
            if isinstance(cloud_raw["personal_context_enabled"], bool):
                cloud["personal_context_enabled"] = cloud_raw["personal_context_enabled"]
            else:
                _record_error(errors, "ask_apex.cloud.personal_context_enabled must be a boolean")
        if "model" in cloud:
            reconciled_effort = reconcile_cloud_reasoning(
                cloud["model"], cloud.get("effort")
            )
            if reconciled_effort is not None:
                cloud["effort"] = reconciled_effort
        hosted_raw = cloud_raw.get("hosted_tools")
        if isinstance(hosted_raw, dict):
            _record_unsupported_agent_fields(
                hosted_raw,
                allowed={"google_search", "google_maps", "x_search"},
                path="ask_apex.cloud.hosted_tools",
                layer_name=layer_name,
                errors=errors,
            )
            ...
            if hosted:
                cloud["hosted_tools"] = hosted
        elif hosted_raw is not None:
            _record_error(errors, "ask_apex.cloud.hosted_tools must be an object")
        if cloud:
            result["cloud"] = cloud
    ...
    if isinstance(local_raw, dict):
        _record_unsupported_agent_fields(
            local_raw,
            allowed={"designation", "model", "context_window", "reasoning_mode", "personal_context_enabled"},
            path="ask_apex.local",
            layer_name=layer_name,
            errors=errors,
        )
        local: dict[str, Any] = {}
        designation = local_raw.get("designation")
        if isinstance(designation, str):
            cleaned = designation.strip()
            if 1 <= len(cleaned) <= 32:
                local["designation"] = cleaned
            elif designation.strip() == "":
                local["designation"] = "Felis"
            else:
                _record_error(errors, "ask_apex.local.designation is not valid")
        elif designation is not None:
            _record_error(errors, "ask_apex.local.designation must be a string")
        model = local_raw.get("model")
        if isinstance(model, str) and get_model_profile(model.strip()) is not None:
            profile = get_model_profile(model.strip())
            if profile is not None and profile.runtime == "local":
                reconciled_model = reconcile_local_model(
                    model.strip(),
                    dev_mode=is_dev_mode(),
                )
                local["model"] = reconciled_model
            else:
                _record_error(errors, "ask_apex.local.model is not valid")
        elif model is not None:
            _record_error(errors, "ask_apex.local.model is not valid")
        context_window = local_raw.get("context_window")
        if isinstance(context_window, int) and not isinstance(context_window, bool):
            local["context_window"] = context_window
        elif context_window is not None:
            _record_error(errors, "ask_apex.local.context_window must be an integer")
        reasoning_mode = local_raw.get("reasoning_mode")
        if isinstance(reasoning_mode, str) and reasoning_mode in {"none", "focused"}:
            local["reasoning_mode"] = reasoning_mode
        elif reasoning_mode is not None:
            _record_error(errors, "ask_apex.local.reasoning_mode is not valid")
        if "personal_context_enabled" in local_raw:
            if isinstance(local_raw["personal_context_enabled"], bool):
                local["personal_context_enabled"] = local_raw["personal_context_enabled"]
            else:
                _record_error(errors, "ask_apex.local.personal_context_enabled must be a boolean")
        if local:
            result["local"] = local
    ...
    if isinstance(local_result, dict):
        model = local_result.get("model", DEFAULT_LOCAL_MODEL)
        profile = get_model_profile(model) if isinstance(model, str) else None
        runtime = (
            profile.provider
            if profile is not None and profile.runtime == "local"
            else DEFAULT_LOCAL_RUNTIME
        )
        if isinstance(model, str):
            context_window = local_result.get("context_window")
            if isinstance(context_window, int) and not isinstance(context_window, bool):
                local_result["context_window"] = reconcile_local_context_window(
                    runtime,  # type: ignore[arg-type]
                    model,
                    context_window,
                )
            reasoning_mode = local_result.get("reasoning_mode")
            if isinstance(reasoning_mode, str) and reasoning_mode in {"none", "focused"}:
                local_result["reasoning_mode"] = reconcile_local_reasoning_mode(
                    model,
