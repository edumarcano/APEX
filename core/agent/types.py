...
class TokenUsage(BaseModel):
    ...
    input_tokens: int | None = Field(
        default=None,
        ge=0,
        description="Prompt tokens sent to the provider model (including cached prompt tokens).",
    )
    cached_input_tokens: int | None = Field(
        default=None,
        ge=0,
        description="Subset of prompt tokens served from provider prompt cache.",
    )
    output_tokens: int | None = Field(
        default=None,
        ge=0,
        description="Visible completion tokens generated by the provider model.",
    )
    reasoning_tokens: int | None = Field(
        default=None,
        ge=0,
        description="Hidden internal reasoning tokens generated by the provider model.",
    )
    total_tokens: int | None = Field(
        default=None,
        ge=0,
        description="Total tokens accounted for this turn or query.",
    )

    @model_validator(mode="after")
    def _compute_total_tokens(self) -> TokenUsage:
        if self.total_tokens is not None:
            return self
        inputs = self.input_tokens
        outputs = self.output_tokens
        reasoning = self.reasoning_tokens
        if inputs is None and outputs is None and reasoning is None:
            return self
        total = (inputs or 0) + (outputs or 0) + (reasoning or 0)
