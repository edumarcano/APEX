...
- Added a process-local telemetry snapshot service (`core/telemetry`) with independent full and per-connector refresh, freshness tracking, and stale-on-failure retention, exposed via `GET`/`POST /api/v1/telemetry` and wired into typed HUD wing cards with Refresh All and per-connector refresh.
- Replaced the hard Wi-Fi/power/cooldown gate with advisory `POST /api/v1/preflight`, returning stable warning and blocker codes for configured-network mismatch, battery use, rapid refreshes, cloud disclosure, credentials, and local-model resource contention; hard blockers remain reserved for conditions that would prevent an operation from running correctly.
- Reworked the HUD into an independently activated Overview: Start APEX and Start with Briefing replace the single synthesis trigger, Ask APEX is available on activation without clearing sessions, and Insights gains a Generate Briefing action from live telemetry.
- Added five explicit briefing modes. Comet (Gemini, default), Lynx, Acinonyx, and Neofelis (local Ollama profiles), and Structured Digest (deterministic, model-free). Selectable from a global header control and via `POST /api/v1/trigger` mode override or the new `POST /api/v1/briefings/generate` snapshot-based endpoint.
- Added Off/Manual/Automatic voice delivery modes (`voice.mode`) with a dedicated `POST /api/v1/voice/speak` endpoint, a Speak/Replay icon action on the Briefing tab, and automatic delivery wired through the briefing pipeline.
- Replaced prose-based confidence inference with typed `ConnectorResult` scoring and an equal-weight `sync_health_score`, surfaced through `connector_health` rows in the digest and a Sync Health popup in the status footer; `confidence_score` and `failed_connectors` remain as compatibility fields.
- Added `GET /api/v1/health/live` and `GET /api/v1/health/ready` probes, used by the launcher for startup readiness instead of the compatibility `GET /` route; readiness checks the runtime settings store and a lightweight SQLite query without calling optional external providers.
...
- Added `core/telemetry/` for independent snapshot collection, merge, and per-connector freshness evaluation, decoupled from briefing orchestration.
- Extended `SynthesisRouter` and `core/synthesis/` with explicit `BriefingMode` routing (Comet/Lynx/Acinonyx/Neofelis/Structured Digest), mapping legacy `cloud`/`local`/`raw` strategy values to Comet/Acinonyx/Structured Digest compatibility aliases.
- Made SQLite writes transactional with timezone-aware UTC timestamps on new rows, while preserving read compatibility with legacy timezone-naive timestamps.
- Propagated a `run_id` through pipeline status, worker threads, persisted briefing metadata, and log context for cross-component correlation.
- Replaced operational `print` statements with module loggers, sanitized assistant/tool failure messages, and classified briefing history quality (`valid`/`legacy`/`malformed`/`zero_health`) with malformed rows logged by ID and category only, never by content.
...
- Bumped the settings contract to schema version 3, adding `briefing.default_mode` and `voice.mode` to the patchable settings surface.
- Extended `POST /api/v1/trigger` with an optional mode override; it now force-refreshes the telemetry snapshot, synthesizes with the requested or configured default mode, and only persists/speaks per production/voice-mode rules, while remaining a supported full-pipeline compatibility route.
...
- Added `docs/privacy.md` covering the local trust boundary, briefing/assistant data handling, persistence, credentials, and logging.
- Documented independent runtime paths, process-local telemetry, advisory preflight, liveness/readiness probes, UTC timestamp compatibility, and the model-synthesis/briefing-mode rationale in `docs/decisions.md`.
- Updated `docs/api.md` and `docs/architecture.md` for the health probes, telemetry/preflight/voice endpoints, schema version 3, Sync Health scoring, and the `core/api/` package layout; updated `docs/design-system.md` with briefing/voice control conventions.
- Updated `README.md` for the advisory preflight model, Sync Health scoring, locked `uv` dependency management, and the loopback-only trust boundary; marked v1.17.0 complete in `docs/roadmap.md` under Runtime Hardening & Decoupling.
