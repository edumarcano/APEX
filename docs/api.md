...
**Client-display exposure:** `tool_outputs` exists so the HUD can render structured result cards (forecast tables, standings, calendar entries, Gmail results, and approved MCP results) without re-deriving them from `answer` text. Exposure is controlled by each capability's `expose_to_client_display` flag. The ten native capabilities currently exposed are `get_weather_forecast`, `get_f1_driver_standings`, `get_f1_season_calendar`, `get_upcoming_calendar_events`, `get_active_reminders`, `get_briefing_history`, `search_gmail`, `get_gmail_message`, `list_microsoft_todo_lists`, and `list_microsoft_todo_tasks`. The tracked GitHub, Brave Search, and Alpha Vantage presets also enable generic client presentation for their explicitly allowlisted tools; arbitrary MCP server entries remain hidden by default.
...
**Bounded tool-calling loop:** Execution is capped by the active profile's `max_tool_turns` and `max_tool_calls` (see [Cloud Agent Profiles](architecture.md#cloud-agent-profiles) in the architecture reference). The last permitted model turn is answer-only with tools withheld, ensuring that the loop can summarize results already collected instead of requesting a call that cannot receive a follow-up turn. Reaching either limit ends the loop and returns the last model text with `error` populated, rather than looping indefinitely or failing the request.
