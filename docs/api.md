...
Football reads up to five upcoming fixtures per followed team within fourteen days, including `SCHEDULED` and `TIMED` matches. When every team read succeeds, no qualifying fixtures reports `healthy` and "No upcoming football fixtures." A failed team read reports `degraded` if fixtures remain from another team or the cache, and `unavailable` otherwise. Missing teams or credentials also report `unavailable`.
