...
- Use `useApexData()` for boot configuration and reminders. Use focused hooks for their respective concerns instead of adding to `useApexData()`: `useAppActivation()` for standby/activated lifecycle, `usePreflight()` for preflight warning/blocker dialog flow, `useTelemetrySnapshot()` for on-demand telemetry refresh, `useBriefingPipeline()` for trigger/status polling and briefing/digest state, `useCortex()` for Agent status, verification, polling, and local-model lifecycle, `useCortexRuns()` for recent runs, active-run polling, and cancellation, `ApexAssistantRuntime` for conversation/thread state and one-turn submission, `useActions()` for Cortex action list, audit detail, and versioned controls, `useContextInspector()` for personal-context records, retrieval preparation, reconciliation, and review, `useContextVault()` for vault settings, preview, and publication controls, `useToolCatalog()` for catalog/profile/selection state, and `useToolPreflight()` for next-request token estimates.
- Keep `useMarketData()` focused on cache-backed Market display data. It reloads when telemetry publishes a Market collection revision and must not add a competing provider refresh or polling loop. Do not force unrelated state into `useApexData()` or into a hook that owns a different concern.
