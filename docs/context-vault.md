...
Disabling export stops updates and leaves generated files in place. To remove APEX-managed copies, disable export first and select **Remove generated copies**; APEX removes only files it tracks and does not recursively delete the destination. Removing a record from the effective selection while export stays enabled removes its managed note on refresh. Changing `APEX_CONTEXT_VAULT_PATH` leaves copies at the previous root for deliberate cleanup, and Vault status reports retained destinations. Recheck Drive after any change or removal. Files or answers already copied, indexed, downloaded, or retained by another service may remain there.
