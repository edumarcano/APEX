...
`status` checks backend readiness, including configuration and database access. `agents` lists the visible Agents and their current availability. `ask` performs one turn only: it does not retain terminal conversation history or attach the current HUD snapshot. When `--profile` is omitted, the backend chooses the saved default profile for the selected Agent.
