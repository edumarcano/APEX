...
`briefing` uses the normal full refresh-and-generate route. Omitting `--mode` uses the saved briefing default; supported overrides are `panthera`, `lynx`, and `structured_digest`.
