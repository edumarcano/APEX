...
| Briefing generation | Selected, size-limited facts are built locally | Focused sends metadata-only email snippets and selected facts to OpenRouter; Flash uses the fixed local Felis llama.cpp runtime | Normal-mode transcript/digest in SQLite | Flash or Structured |
...
Focused briefings always use Panthera through OpenRouter DeepSeek V4 Flash with High reasoning, while Flash uses its fixed Gemma E2B llama.cpp briefing path with reasoning disabled. The cloud boundary includes up to eight recent inbox candidates with sender, subject, timestamp, and sanitized provider snippet, never message bodies. Interactive Agent model selection does not change briefing generation. Display text, Agent tools, and conversation history are not sent as briefing input. Generated output is checked before use; invalid model output falls back to Structured built from the same facts.
...
The Panthera briefing path always sends briefing input to OpenRouter DeepSeek V4 Flash and can include personal facts such as calendar events, reminders, and limited email subjects. Every request requires `provider.zdr=true`, `provider.data_collection="deny"`, and `provider.require_parameters=true`; a route without those guarantees is unavailable rather than used. The interactive Panthera model selection does not alter this fixed briefing route.

This controls downstream routing, not OpenRouter account-level prompt logging or data-use opt-ins, which operators must leave disabled before sending personal data. OpenRouter's ZDR definition can still permit in-memory caching. See [OpenRouter ZDR](https://openrouter.ai/docs/guides/features/zdr) and [data collection](https://openrouter.ai/docs/guides/privacy/data-collection).
...
- OpenRouter DeepSeek V4 Flash requests, including Panthera briefings, always require `provider.zdr=true`, `provider.data_collection="deny"`, and `provider.require_parameters=true`; a route without those guarantees is unavailable rather than used.
