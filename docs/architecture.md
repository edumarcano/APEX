...
Displays insight bullet strings from `DigestPayload.insights` in the center column above the `ApexLogo`. When activated, the Insights card exposes a Briefing Mode selector, **Generate Briefing**, **Refresh All & Generate**, and optional **Speak / Replay** (when `voice.mode` is not `off`). Generate uses `POST /api/v1/briefings/generate` with the current `snapshot_id`; Refresh All & Generate proceeds only when refresh returns a new snapshot. Independent voice delivery is tracked by `useVoiceDelivery`, which disables replay during playback and surfaces stable API failures in the card. When `status === 'success'`, a "History" button appears in the card header; clicking it opens `HistoryLedgerModal` as a portal-mounted dialog. The modal fetches `GET /api/v1/briefings/history` on mount and renders each `BriefingHistoryRecord` with a formatted timestamp, briefing prose, and insight bullets. The modal closes on backdrop click or `Escape`.
