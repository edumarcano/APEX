...
- **Explicit local tool scopes.** Local queries are tool-free by default. `tool_scope` resolves one of seven operator-selected bundles (`schedule`, `weather`, `f1`, `mail`, `search`, `market`, `briefings`); the loop offers and dispatches only that bundle. The local `search` projection exposes one Brave web-search tool with only `query` and bounded `count`, without mutating the registered MCP descriptor used by cloud profiles. Cloud profiles continue to receive the full automatic registry. GitHub and `/auto` are not local scopes.
