...
Production native capabilities are primarily read-only. `create_microsoft_todo_task` is the first approval-gated native write: an Agent can propose it, but only local action approval can execute it. The action executor persists only the returned list and task identifiers before its verifier performs an exact Graph read-back; retries recover that evidence from the immutable action ledger instead of replaying the write. MCP discovery registers only allowlisted tools with explicit local risk classifications. Imported tools are namespaced on collision, bounded before model and client display, and never re-exported as an APEX MCP server.
