...
An opt-in process on loopback can expose only activity submission through JSON HTTP and Streamable HTTP MCP. It opens the same activity store as local CLI and file import and checks the registration for every tool call or JSON request. Local mode resolves the generic `operator` principal and must never be tunneled. Cloudflare mode verifies the forwarded Access assertion at this boundary, selects one configured audience-and-subject binding, then passes only its generic client principal into the same activity service. Cloudflare headers, raw claims, signing keys, and OAuth state never enter report storage, review logic, or the activity domain model. The process has no main API routes, Cortex initialization, connectors, report reads, resources, prompts, actions, or proxy behavior. Its in-process rate limit is intentionally local to this one optional gateway.
