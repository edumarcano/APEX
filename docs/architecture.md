...
`BriefingControls` replaces the former synthesis-status pill beside the hardware metrics. Its selector groups cloud, local, and model-free briefing modes while reusing the assistant profile status feed for credentials, Ollama availability, installed models, resource thresholds, and inference contention. The adjacent split control generates from current telemetry or refreshes all connectors first. The selection is a session override; Settings remains the persisted default.
