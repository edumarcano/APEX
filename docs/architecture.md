...
- **Explicit local tool scopes.** Local queries are tool-free by default. `tool_scope` resolves one of seven operator-selected bundles (`schedule`, `weather`, `f1`, `mail`, `search`, `market`, `briefings`); the loop offers and dispatches only that bundle. Cloud profiles continue to receive the full automatic registry. GitHub and `/auto` are not local scopes.
