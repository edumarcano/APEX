...
- Creates one `MCPClientManager` even when MCP is disabled so Runtime Settings can enable providers without restarting APEX. Discovery runs as non-blocking per-server tasks so offline MCP servers never delay readiness. Live reconciliation serializes provider changes, unregisters tools before transport shutdown, and prevents cancelled discovery tasks from registering stale capabilities. On shutdown the manager closes every transport and unregisters imported capabilities.
...
- **`num_predict` and tool withholding.** `OllamaProvider` sets `num_predict` to the profile's `tool_select_max_tokens` when tools are offered, or `final_answer_max_tokens` when they are not. `run_agent_loop()` withholds tools entirely on the last permitted turn for every provider and appends a final-answer instruction, forcing a text answer instead of a tool call that could never execute.
- **Truncated tool-select retry.** If a tool-offering turn hits the `num_predict` ceiling without producing a tool call (`done_reason: "length"`), the same turn is regenerated once without tools. The payload is rebuilt through the normal budgeter using `final_answer_max_tokens`, so additional complete history interactions are removed when the larger output reservation requires them.
- **Explicit local tool scopes.** Local queries are tool-free by default. `tool_scope` resolves one of eight operator-selected bundles (`schedule`, `weather`, `f1`, `mail`, `search`, `market`, `briefings`, `todo`); the loop offers and dispatches only that bundle. The local `search` projection exposes one Brave web-search tool with only `query` and bounded `count`, without mutating the registered MCP descriptor used by cloud profiles. Cloud profiles continue to receive the full automatic registry. GitHub and `/auto` are not local scopes.
- **Context budgeting.** Before each Ollama call, APEX estimates the serialized prompt plus output allowance and a safety margin against `num_ctx`. It removes old user-led interactions as complete units. A recognized provider overflow is retried once with the current interaction only; tool schemas remain unchanged. Responses report sanitized counts through `local_context_usage`.
...
|---|---|---|---|---|---|---|---|
| `comet` | Apex Comet | `gemini-3.5-flash-lite` | fast | stable | minimal | `min(6, GEMINI_AGENT_MAX_TURNS)` | `min(10, GEMINI_AGENT_MAX_TOOL_CALLS)` |
| `nova` | Apex Nova | `gemini-3.5-flash` | balanced | stable | low | `min(4, GEMINI_AGENT_MAX_TURNS)` | `min(6, GEMINI_AGENT_MAX_TOOL_CALLS)` |
| `pulsar` | Apex Pulsar | `gemini-3.6-flash` | advanced | stable | medium | `min(4, GEMINI_AGENT_MAX_TURNS)` | `min(6, GEMINI_AGENT_MAX_TOOL_CALLS)` |

Per Google's Gemini 3.x guidelines, sampling parameters such as `temperature` are omitted from Gemini profile definitions and `GenerateContentConfig` requests so models run with native model defaults. `GEMINI_AGENT_MAX_TURNS` (1–6, default 6) and `GEMINI_AGENT_MAX_TOOL_CALLS` (1–10, default 10) are read from `config.json` `gemini.agent_max_turns` / `gemini.agent_max_tool_calls` in `core/config.py`. Profile-specific ceilings reflect the configured free-tier request limits: Comet receives the larger Flash-Lite allowance, while Nova and Pulsar share the more conservative full-Flash allowance. The last permitted turn is answer-only for every provider so an exhausted loop cannot spend its final model request on another tool call.
...
Local `max_tool_turns` and `max_tool_calls` are profile constants independent of the Gemini operator ceiling: Lynx uses `2` / `3`, while Acinonyx and Neofelis use `3` / `4`. All three profiles default `think=False` (Ollama's chain-of-thought phase is disabled) and `default_temperature=0.2` (`0.1` for Neofelis). RAM and CPU limits are percentage thresholds evaluated by the resource gate before a cold load; they are configurable per profile via `config.json` `ollama.resource_gates.{lynx,acinonyx,neofelis}.{ram_limit,cpu_limit}`, defaulting to the values above.
...
**`core/mcp/`** — FastMCP-backed external MCP client runtime. `load_mcp_config()` reads the non-secret `mcp` section from `config.json` overlaid by `config.local.json`. Credentials are referenced only by environment-variable names (`auth_env`, `header_env`) and resolved at connect time; OAuth providers use FastMCP's browser flow with token and client state stored in the operating-system credential manager. `MCPClientManager` is always owned by the FastAPI lifespan and connects enabled servers over HTTP (`StreamableHttpTransport`) or stdio (`StdioTransport`). Its serialized `reconfigure()` path applies saved Runtime Settings without restarting APEX, affects only changed providers, and uses provider generations to reject stale discovery results after rapid toggles. Disabling unregisters capabilities before closing transports, so no new calls can begin.

Discovery registers only names present in that server's `tool_allowlist` with an explicit local `tool_risks` classification (`read`, `write`, or `destructive`). Optional `tool_argument_maximums` narrow numeric arguments before their schema reaches a model. Imported capabilities use `origin="mcp"`, `expose_to_assistant=True`, and `expose_to_mcp_server=False`; client-display exposure remains an explicit per-server choice. Empty allowlists and allowlisted tools without a local risk classification register nothing even if the remote advertises tools. Sync agent invokes bridge onto the application event loop through `asyncio.run_coroutine_threadsafe` with bounded connection, call, and output limits. Offline or misconfigured servers mark `degraded` / `authentication-required` status without blocking API startup or rolling back saved settings. `GET /api/v1/mcp/status` returns sanitized diagnostics only.

The tracked provider pilots are disabled by default. GitHub uses the official remote read-only endpoint plus a local read-only allowlist; Brave runs the official Node stdio package and admits only web/news search; Alpha Vantage uses the hosted OAuth endpoint for a bounded market-research set. The existing Alpha Vantage API client remains the independent cache-first source for the HUD ticker.
...
Displays only synthesis-produced insight strings from `DigestPayload.insights` in the center column above the `ApexLogo`; reminders remain in their dedicated panel. The briefing mode selector lives in the global header and uses the assistant profile availability matrix for model-backed modes. Beneath the logo, a split Synthesize command either uses the current `snapshot_id` or refreshes all telemetry before synthesizing from the returned snapshot. The Insights card reserves its body for synthesis insights and transcript text; optional Speak / Replay appears as an icon only on the Briefing tab. Independent voice delivery is tracked by `useVoiceDelivery`, which disables replay during playback and surfaces stable API failures with the transcript. When `status === 'success'`, a "History" button appears in the card header; clicking it opens `HistoryLedgerModal` as a portal-mounted dialog. The modal fetches `GET /api/v1/briefings/history` on mount and renders each `BriefingHistoryRecord` with a formatted timestamp, briefing prose, and insight bullets. The modal closes on backdrop click or `Escape`.
...
The briefing controls replace the former synthesis-status pill beside the hardware metrics. The header selector groups cloud, local, and model-free briefing modes while reusing the assistant profile status feed for credentials, Ollama availability, installed models, resource thresholds, and inference contention. The split Synthesize command beneath the logo uses current telemetry or refreshes all connectors first. The selection is a session override; Settings remains the persisted default.
