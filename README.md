...
Optional MCP provider pilots are disabled by default. GitHub requires `GITHUB_PERSONAL_ACCESS_TOKEN`; Brave Search requires `BRAVE_API_KEY`, Node.js 22 or later, and `npx`. Alpha Vantage MCP uses browser-based OAuth and stores authorization in the operating-system credential manager. Enable the MCP runtime and individual presets through `config.local.json`; see [the MCP status and configuration reference](docs/api.md#get-apiv1mcpstatus). These assistant integrations do not replace scheduled news collection or the native cached market ticker.
