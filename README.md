...
Agent queries can use approved read capabilities for live data, briefing history, Gmail, Microsoft To Do, and optional MCP (Model Context Protocol) providers. Bounded Microsoft To Do create, update, completion, reopening, and deletion actions can be proposed, then reviewed in Cortex before local operator approval and independent verification. Cloud and local Agents share one provider-neutral capability layer and one Tools selector; the selected names narrow Agent policy without changing MCP authorization.
